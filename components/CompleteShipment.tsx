...
interface CompleteShipmentProps {
  completeModel: boolean;
  setCompleteModel: (value: boolean) => void;
  completeShipment: (completeShip: CompleteShipmentParams) => Promise<void>;
}
...
const CompleteShipment: React.FC<CompleteShipmentProps> = ({ completeModel, setCompleteModel, completeShipment }) => {
      ...
      <div className='flex items-center min-h-screen px-4 py-8'>
        <div className='relative w-full max-w-lg p-4 mx-auto bg-white rounded-md shadow-lg'>
          <div className='flex justify-end'>
            <button>
              <MdCancel onClick={() => setCompleteModel(false)} size={32} className='cursor-pointer' />
            </button>
          </div>
